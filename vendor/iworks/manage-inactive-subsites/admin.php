...
class IworksManageInactiveSubsitesAdmin extends IworksManageInactiveSubsites {
	...
	public function add_network_settings_submenu() {
		$page = add_submenu_page(
			'sites.php',
			__( 'Manage Inactive', 'manage-inactive-subsites' ),
			__( 'Manage Inactive', 'manage-inactive-subsites' ),
	...
	public function add_notice_with_instruction() {
		...
		$notice .= sprintf(
			__( 'Current configuration: after <strong>%1$s</strong> of inactivity site will be <strong>%2$s</strong>.', 'manage-inactive-subsites' ),
			$period,
			$action
		);
		/**
		...
		$notice .= PHP_EOL . PHP_EOL;
		$notice .= sprintf(
			__( 'You can easy change configuration, please go to Network Admin » Sites » <a href="%s">Manage Inactive</a> and change current configuration.', 'manage-inactive-subsites' ),
			esc_url( add_query_arg( 'page', 'manage-inactive-subsites-settings', network_admin_url( 'sites.php' ) ) )
		);
